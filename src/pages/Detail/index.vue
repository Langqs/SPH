          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: minitem.isChecked == 1 }"
                  v-for="minitem in item.spuSaleAttrValueList"
                  :key="minitem.id"
                  @click="changeActive(item.spuSaleAttrValueList, minitem)"
                >
                  {{ minitem.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  computed: {
    ...
    // ...mapState({DetailList:'getDetailList'}),
    skuImageList() {
  ...
  //方法
  methods: {
    changeActive(saleAttrValueList, saleAttr) {
      saleAttrValueList.forEach((element) => {
        element.isChecked = "0";
      });
    ...
    async addCart() {
      try {
        ...
        //会话存储，将产品信息传到加入购物车成功页
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        //跳转路由,简单数据通过query传递，复杂数据通过[会话存储]
